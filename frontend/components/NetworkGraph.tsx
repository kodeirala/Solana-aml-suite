...
export default function NetworkGraph({ address, connections, onNodeClick }: NetworkGraphProps) {
  ...
  useEffect(() => {
    ...
    node.filter((d: Node) => !!d.is_center)
      ...
      .attr('stroke', 'rgba(245,197,24,0.3)').attr('stroke-width', 1).attr('stroke-dasharray', '4 4');

    node.filter((d: Node) => d.risk >= 70)
      .append('circle').attr('r', 22).attr('fill', 'none')
      .attr('stroke', 'rgba(255,51,51,0.4)').attr('stroke-width', 1);
    ...
    node.filter((d: Node) => !!d.is_center)
      .append('text').attr('text-anchor', 'middle').attr('dominant-baseline', 'middle')
      .attr('font-size', '14px').text('⬡').attr('fill', '#f5c518');

    node.filter((d: Node) => !d.is_center)
      .append('text').attr('text-anchor', 'middle').attr('dominant-baseline', 'middle')
      .attr('font-size', '9px').attr('font-family', 'Orbitron, sans-serif').attr('font-weight', 'bold')
      .attr('fill', (d: Node) => d.risk >= 70 ? '#ff3333' : d.risk >= 40 ? '#f5c518' : '#00ff88')
      .text((d: Node) => d.risk);
        ...
        <div className="absolute pointer-events-none z-10 p-3 rounded border" style={{ left: tooltip.x + 12, top: tooltip.y - 10, background: '#0a0a0f', borderColor: 'rgba(245,197,24,0.3)', boxShadow: '0 0 20px rgba(245,197,24,0.1)', maxWidth: '240px' }}>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {tooltip.node.is_blacklisted && (
              <span style={{ background: 'rgba(255,0,0,0.2)', color: '#ff0000', border: '1px solid rgba(255,0,0,0.3)', padding: '2px 8px', borderRadius: '4px', fontSize: '10px' }}>
                🚫 BLACKLISTED
              </span>
            )}
          </div>
        ...
        <div className="space-y-1.5">
          {[
            ...
            <div key={item.label} className="flex items-center gap-2">
              ...
              <span style={{ color: 'rgba(200,200,212,0.5)', fontSize: '9px' }}>{item.label}</span>
            </div>
          ))}
        </div>
        <p style={{ color: 'rgba(200,200,212,0.3)', fontSize: '9px', marginTop: '8px' }}>SCROLL TO ZOOM · DRAG TO PAN · CLICK NODE</p>
      </div>
